...
def login():
    ...
    if form.validate_on_submit():
        ...
        ## by setting current_user to currently logged in user
...
def before_request():
    if current_user.is_authenticated:
        ...
        ## no db.session.add() because whenever the current_user is referenced,
        ## Flask automatically queries the db that puts the target user
        ## in the database session. Since last_seen is already a filled column,
        ## there's no need to add it anymore (this is just a simple modification)
        db.session.commit()
